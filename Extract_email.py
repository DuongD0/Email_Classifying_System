...
class Dataset_Creation:
    ...
    def generate_csv_file(self, valid_emails):
    	...
    	output_path = DATASETS_DIR / 'final_dataset.csv'
...
datasets_base_path = DATASETS_DIR
fraud_path = datasets_base_path / 'fradulent_emails.txt'
spam_normal_path = datasets_base_path / 'spam_normal_emails.csv'

if not fraud_path.exists() or not spam_normal_path.exists():
    raise FileNotFoundError('Required dataset files are missing. Please ensure "fradulent_emails.txt" and "spam_normal_emails.csv" exist in the Datasets directory.')

emails_dataset = fraud_path.read_text(encoding='latin-1')
...
emails_dataset2 = spam_normal_path.read_text(encoding='latin-1').splitlines()
